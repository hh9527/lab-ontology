...
| Historical qualification, separate observation | Q0105-Q0110 | Count samples above a raw-value threshold in one window, then aggregate or list samples in another window for the *same qualified device*. These are two independent scoped uses of a KPI carrier, potentially with different calendar units. Positive acceptance: separate source aliases/scopes, device identity preserved, external UTC anchor, no multiplication of observation samples by qualifying samples. Negative: treating a per-sample threshold as HAVING on the observation aggregate, or applying the qualifying window to the displayed trend. Q0108 uses 30 days for qualification and one calendar month for observation: these are not synonymous. |
...
The present acceptance checks SQL structure, complete composite correlation,
binding order, model-declared Avg versus Sum, and negative relation/measure
cases; it does not yet execute the generated plan against a two-by-three
sample fixture. Qualification by a raw-sample predicate before COUNT (Q0105)
is a distinct next pressure point.
