...
| Child-grain KPI summarized at device | Q0328, Q0332, Q0342 | Q0332/Q0342 now have a bounded/all-history `observe_related` acceptance over AP-radio-SSID samples: IC metadata describes `resId` as SSID ID and `parentId` as AP ID, while the product data link explicitly joins `(parent_id,tenant_id)` to Device `(id,tenant_id)` (the corpus SQL omits tenant). The pressure Model declares candidate sample grain `(res_id,tenant_id,ts)`, retains IC's published Sum and separately declares a raw-sample Avg; a named Safe relation and hidden full AP grain group pooled raw samples. SQLite checks unequal sample counts across two SSIDs (pooled 25 versus incorrect equal-SSID mean 20), window/all-history means (25/38), three same-name AP identities (25/70/5) and foreign-tenant sample exclusion. Missing tenant relation key and unrelated relation fail. The `resId` isPK metadata is lineage evidence, not proof that `(resId,tenantId,ts)` is unique in production; the sample grain remains a contract to validate. Q0328's PON-port child sample Max is still open. An AP-wide latest whole sample is not implied when several SSIDs can share a timestamp. |
