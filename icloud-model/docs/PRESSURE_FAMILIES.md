...
| Bilingual knowledge-point discovery | NetworkSlot Model and Chinese corpus | IC names the published Slot `Slot`/`单板` and its Boolean field `Subboard`/`是否子卡`. Dataset/member aliases expose these names against stable IDs in both full and subject discovery; measure and named-relation aliases obey the same declaration contract. The intent still names only stable IDs. Chinese explanatory summaries and locale negotiation remain separate questions. |
