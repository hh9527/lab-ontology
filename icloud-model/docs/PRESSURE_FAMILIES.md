...
| Peer-qualified raw KPI trend | Q0075-Q0077 | Open capability gap: `lower_peer` filters a PhysicalLink root, while `observe_related` observes a Metric root and cannot qualify the owning Device through a distinct peer's link. The question and SQL ask for raw `(value,ts)` rows ordered by `ts`, **not** daily bucket aggregates; the older IC audit's prose about missing time buckets is not the actual acceptance shape. Q0075's target SQL even filters the neighbor as router although its question says switch, so its SQL is not a business definition. Acceptance must use a named KPI→owner Safe relation and a role-checked link with complete A/Z device identity; one correlated link EXISTS must keep each sample once despite duplicate links and match both orientations without combining endpoints from different links. Externally supplied UTC bounds, two namesake owners, equal timestamps, non-neighbor and foreign-tenant endpoints, and wrong peer roles are required positive/negative probes. A fan-out join would duplicate samples, while independent A/Z OR predicates could fabricate a path. |
