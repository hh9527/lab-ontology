...
| Site-alarm data link uses `Site.site_id = Alarm.strext13`, but alarm `strext13 = "3"` means a system alarm, not a site reference | `alarm_site_reference` declares the Safe equality and a typed, source-field `Ne("3")` guard on that named relation, not a global alarm scope | Site-alarm join/EXISTS/constraint lowering binds and applies the same guard even if a Site with ID `"3"` exists; global alarm counts still include the system alarm. Bad fields and value types fail at preparation, and the relation knowledge point links to the guard field. Guarded A/Z business links are explicitly rejected pending their own branch semantics. Source: `governance/event_sets/current_alarm.telora` and `governance/data_links/intra.telora`. |
...
| Server endpoint identity versus link tenant alignment | IC marks ServerDevice.id as PrimaryKey, while `server/entity_set_links/cross/network.telora` declares only `id = aNeResId` and `id = zNeResId`; both records also carry tenant IDs | The Q0350 pressure model instead declares Server grain `(id,tenant_id)` and includes tenant equality in both A/Z keys. This deliberately excludes same-ID foreign-tenant matches, but product declarations do not prove the extra equality. Before a production model chooses either contract, verify the scope of the PrimaryKey and whether cross-table tenant IDs must agree; do not claim the stricter pressure relation is the published IC relationship. Ontology can lower either explicitly declared key shape. |
| Tenants with more than five uncleared alarms | Alarm `CLEARED` maps business `uncleared` to integer wire 0; named Alarm→Tenant ownership maps `TENANTID` to Tenant's `TENANT_ID` identity | Graph groups Alarm counts by complete Tenant identity, applies HAVING and counts the surviving groups. This counts existing Tenant entities; the corpus groups raw `TENANTID` without joining Tenant and may also count orphan IDs. Referential integrity has not been established, so these meanings must remain distinct. |
| `occur_utc` is the alarm's authoritative UTC time field; `arrive_utc` is a distinct UTC arrival clock; `occur_time` is local | Event grain and UTC/Local field encoding are declared; `utc_window` validates canonical UTC seconds and lowers on `occur_utc`, while `utc_field_window` explicitly targets a declared secondary UTC field such as `arrive_utc` | `UtcEpochMillis` and `utc_epoch_window` now provide a separate integer-clock contract with externally supplied millisecond bounds, including explicit secondary-clock selection and rejection of local/text clocks. IC's `alarm_wire_sql.telora` maps raw-long `OCCURUTC`/`ARRIVEUTC` to PostgreSQL `timestamptz` but explicitly leaves epoch-unit decoding unresolved; corpus SQL compares `OCCURUTC` against epoch milliseconds. Our SQLite text-clock fixture and the new independent integer-clock test do not validate that production mapping. |
| A/Z locate physical endpoints; this connection is bidirectional | `peer_hub_relation_fields` binds exchangeable endpoints to the two named A/Z Safe relations | `peer` matches either orientation on one link while retaining full `(id,tenant_id)` identity and excluding a self-pair. `observe_peer_related` uses the same declared relation roles in a correlated EXISTS, preserving raw KPI samples even with duplicate links. Directional connections retain the separate `directed_peer_hub_relation_fields` declaration. |
| A/Z roles exist on both one-way and bidirectional links | IC declares `PhysicalLink.direction` with physical values `bidirectional` and `unidirectional` | The exchangeable `physical_link_peer_device` business link carries a hub guard `Eq(direction,"bidirectional")`; separate `physical_link_downstream_device` is Directed A→Z with `Eq(direction,"unidirectional")`. Graph JOIN and correlated EXISTS bind the guard, while named A/Z endpoint relations remain available regardless of direction. Both guarded business links are discoverable by exact topic; a static unguarded Undirected declaration would wrongly accept one-way links in both orientations. |
...
| ONU creation boundary and receive-bandwidth trend use different clocks | IC PonDevice.id is PrimaryKey, createTime is a local-facing bigint epoch-ms, MetricOnu.ts is the UTC sample clock, and its if_in_band_rate metric declares Avg/% | Q0044-shaped row trend lowers from the named ONU KPI dataset even when PON name is projected first; externally resolved creation milliseconds and UTC sample bounds filter their respective sources. The corpus `PonDeviceOnuKPI` physical table remains an acceptance fixture, not a confirmed product SQL mapping. |
...
| Frame belongs to a device; device KPI samples belong to a device through `(resId, tenantId)` | Frame→Device binds `(refParentNE,tenantId)=(id,tenant_id)` so its Safe path covers Device's declared composite grain; KPI rows have their own composite ownership relation | Graph can aggregate Device-owned samples per `(Frame,Device)` context without claiming frame ownership; the same Device total is intentionally repeated for each matching Frame and is not additive across Frames. This requires both hidden identities and excludes additional multiplying joins. The frame tenant equality is a pressure-model contract not published by IC's original single-column entity link. |
...
| NetworkSlot publishes `is_daughter_card: Bool` with physical `I_EnterpriseSlot.isDaughterCard: Bool` | Plain Bool `slot_is_daughter_card` exposes only typed Eq/Ne filtering and discovery | Literal JSON booleans bind as booleans; text or integer lookalikes are rejected. The SQLite fixture distinguishes child and regular slots. StorageDisk also declares `virtual_disk: Bool`, but it is not published as a product query field. |
| IC describes published entities and fields in English and Chinese | `network_slot` publishes `Slot`/`单板`; `slot_is_daughter_card` publishes `Subboard`/`是否子卡` through validated knowledge aliases. Measure and relation aliases use the same stable-ID cross-reference | Subject discovery returns both names without accepting the Chinese display name as an intent ID; the dataset index links to the matching dimension and measure IDs. This is name discovery, not locale-specific summary translation. |
...
exchangeable peer route uses `peer_hub_relation_fields` so adding native
