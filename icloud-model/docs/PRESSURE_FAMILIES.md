...
| Child-grain KPI summarized at device | Q0328, Q0332, Q0342 | PON-port and AP-radio-SSID KPI carriers have parent-device identifiers but sample identity and aggregation at child grain must be declared separately. Q0328 asks for a maximum across eligible port samples; Q0332/Q0342 ask for an average across eligible SSID samples, which is not generally the unweighted average of each SSID's averages. IC's AP-radio-SSID mapping exposes `resId`, `parentId`, `tenantId`, `projectId`, `ts` and `connectedTerminals` but does not establish whether `(resId, tenantId, ts)` is a unique sample key or whether `parentId` alone identifies the AP across tenants. Acceptance: a named, full-owner-grain relation, an explicitly declared child sample grain and Avg/Max sample summary, hidden device identity grouping, and a fixture with unequal sample counts across two children and same-ID devices in different tenants. Reject a missing owner key or a purported sample grain that is not demonstrably unique; do not infer an AP-wide latest whole sample from a timestamp shared by multiple children. This is an open modeling/lineage pressure, not a completed production mapping. |
