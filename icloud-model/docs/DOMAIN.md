...
| Site-alarm data link uses `Site.site_id = Alarm.strext13`, but alarm `strext13 = "3"` means a system alarm, not a site reference | Not yet declared: the existing `RelationKey` vocabulary cannot attach a typed source-value exclusion to one named relation without removing that event from every other alarm query | A named site-alarm relation must exclude `"3"` in its own join/EXISTS/constraint semantics even if a Site row with ID `"3"` exists; global alarm counts must still include the system alarm. Missing or mistyped guard, wrong relation roles and attempts to silently substitute a site relation must fail. Discovery must describe the guard on that relation. Source: `governance/event_sets/current_alarm.telora` and `governance/data_links/intra.telora`. |
