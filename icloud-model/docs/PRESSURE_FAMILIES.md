...
| --- | --- | --- |
| Business connection direction from hub state | Q0075-Q0077 peer semantics and IC PhysicalLink.direction | Physical A/Z slots do not determine business direction, and IC records both `bidirectional` and `unidirectional`. `guarded_business_link` now carries a preparation-validated `Eq(direction,"bidirectional")` predicate on the hub. Graph peer JOIN and correlated EXISTS both bind the same guard; a separate one-way A/Z endpoint relation still traverses a unidirectional row. SQLite includes forward/reverse bidirectional rows, forward/reverse one-way rows and a foreign-tenant endpoint. Unknown guard field or wrong value type fails during preparation. Knowledge describes the guard and links to its field; an unconditional Undirected peer would otherwise admit illegal paths. |
