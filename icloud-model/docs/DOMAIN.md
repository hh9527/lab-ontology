...
| A/Z roles exist on both one-way and bidirectional links | IC declares `PhysicalLink.direction` with physical values `bidirectional` and `unidirectional` | The exchangeable `physical_link_peer_device` business link carries a hub guard `Eq(direction,"bidirectional")`; Graph JOIN and correlated EXISTS bind it, while the named A/Z endpoint relations remain available for one-way links. The guard and its field reference are discoverable from the same knowledge point. A static Undirected declaration without the guard would wrongly accept one-way links in both orientations. |
