...
| Link/site qualification must preserve link identity | A/Z Link→Device named Safe relations now match both device ID and tenant ID; Device→Site is FanOut with two OR keys, then Site→Tenant and Link→Tenant align | `qualify_link_site` counts/lists links by a three-hop correlated EXISTS; matching both ends or multiple sites does not multiply the outer link. IC's original entity links mention only device ID, so production data still needs validation of the tenant-alignment assumption; the older peer hub remains a separate cross-tenant audit. |
