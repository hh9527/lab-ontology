#!/usr/bin/env python3
"""Build a deterministic, SQL-group-isolated world_1 experiment split."""
...
def main() -> None:
    ...
    assert len(modeling_sql) == 40
    assert len(evaluation_sql) == 23
    assert modeling_sql.isdisjoint(evaluation_sql)
    assert sum(map(len, modeling_groups)) == 77
    assert sum(map(len, evaluation_groups)) == 43
