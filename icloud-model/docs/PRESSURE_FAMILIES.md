...
| Repairable canonical ID diagnostic | Q0193 WAC classification intent | The IC Model publishes `WAC`, not lowercase `wac` or a physical wire such as `AC`. The closed canonical lookup already rejected unknown IDs, but its message omitted the valid IDs. The structured `fail!` now names the dimension, rejected input, every Model-declared stable ID and its exact `doc.ic` topic; the service diagnostic retains model-source provenance. A Graph regression checks the candidate list. This does not loosen the accepted intent language, infer aliases or expose physical codes as stable IDs. |
