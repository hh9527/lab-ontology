...
| --- | --- | --- |
| Distinct dimension cardinality versus entity count | Q0014-Q0016 | `count_value:{node:"site",dimension:"site_name"}` explicitly counts distinct, non-null declared business values across matched rows, not Site identities. Graph validates dimension ownership/authorization and rejects mixing value count with entity count, visible projection, grouping, ordering or Top-N. The Server→Site FanOut OR fixture checks same-name different-ID sites, a server matching both site keys and a separate server filter: two distinct names, not three sites or four join rows. IC's pressure model does not yet publish `assetNumber`, so the fixture filters by declared `server_name` and does not claim to reproduce Q0014 in full. A question phrased as “number of sites” remains ambiguous when target SQL counts names; the agent must explicitly choose the intended cardinality. |
