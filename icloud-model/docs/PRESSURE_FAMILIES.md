...
| Graph count of OR-related children | Q0050-Q0052 | Graph Intent can now count StorageDevice rows per Site while retaining empty sites: the root is Site, the sole edge is child-to-root FanOut OR, each branch covers the complete Site identity, only root dimensions are selected, and `group_by_identity` explicitly names the root. Child predicates stay inside the LEFT JOIN population, including the canonical storage subtype exclusion. The same proof accepts a composite Device `(id,tenant_id)` when both A/Z branches cover both keys; an incomplete alternative reports its relation, branch number and missing root identity field so the author can repair that equality. This covers grouped child counts, **not** the subsequent HAVING threshold and outer count of qualifying sites; `count_related_groups` above still supplies that complete Q0050-Q0052 shape. |
