...
| Count events grouped by canonical business value | Q0239 | Graph previously required full root identity whenever `count_having` was present, incorrectly rejecting `GROUP BY alarm_severity HAVING count(events)>2`. It now allows an explicit root-dimension value group only when counting root rows, with no identity grouping or `include_empty`; related counts and zero-preserving groups still require identity. For canonical dimensions the Model's declared wire set also guards the source rows: otherwise unknown codes collapse into a projected NULL group and can pass HAVING. SQLite verifies three physical severity `"1"` events yield `critical=3` while three unknown codes and two major events do not create output groups. An ungrouped threshold, child projection and unrelated joins remain rejected. |
