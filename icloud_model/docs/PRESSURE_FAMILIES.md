...
pass the existing global reachability/closure check. `ic/info` still rebuilds
...
FanOut match with an explicit tenant equality in its business relation key;
other cases may independently constrain fan and server to the same Tenant node
inside EXISTS. Wrong relation roles, disconnected nodes, and constraints that would
...
The reconnected relation-first `icloud_model/tests/graph.telora` checks two
...
| Count event-qualified tenant identities | Q0442-Q0443 | IC maps alarm `tenant_id` to physical `TENANTID`, not `TENANT_ID`; that same field participates in device/alarm composite correlation. The cleared canonical domain maps business `uncleared` to integer wire 0. Graph can count Alarm identities grouped by complete Tenant identity, apply bound `count_having > 5`, and `count_groups` the surviving **existing Tenant entities**. Alternatively, root at Alarm and group by the declared `alarm_tenant_id` reference value: this counts non-null IDs, including orphans. The latter cannot count SQL NULL as another tenant: the closed QueryAst `IsNotNull` filter is applied to root-value groups and materialized by both dialects. SQLite checks six uncleared events each for an existing ID, an orphan ID and a missing ID: referenced-ID count is 2, entity count is 1; cleared and under-threshold events do not qualify. The Model's dimension and relation knowledge distinguish these two meanings without asserting referential integrity. |
...
| Component-qualified event observation | Q0392-Q0397 | Event→Server uses the declared Safe `(MEDN,TENANTID)` ownership relation, checked against every field of Server's current `(id,tenant_id)` grain before the Event→Server JOIN; possibly non-unique Fan/PSU→Server relations stay FanOut, admitted only inside correlated EXISTS. Each business containment relation now includes `parentResId=oriResId` and matching component/server tenant IDs, so omission of a separate Tenant constraint cannot borrow a cross-tenant component. Event.CSN is not duplicated by multiple qualifying components or servers with the same oriResId. For per-server event counts, lowering hides both Server identity keys in GROUP BY while exposing the requested name. Removing the Event tenant key or extending Server grain without updating it is diagnosed. Q0392/Q0396 test fans; Q0395/Q0397 test PSU health `unknown` (integer wire -2, not `warning`) and canonical Huawei manufacturer, with externally supplied UTC bounds. SQLite verifies a same-parent foreign-tenant unknown PSU cannot qualify a red event while duplicate matching PSU rows do not duplicate a valid event. IC publishes parent-only physical Fan/PSU links; tenant equality is a pressure-model business ownership contract requiring production validation. The alarm time column is `OCCURUTC`, not `OCCUR_UTC`. |
...
| Count events grouped by canonical business value | Q0234-Q0235, Q0238-Q0239 | Graph previously required full root identity whenever `count_having` was present, incorrectly rejecting `GROUP BY alarm_severity HAVING count(events)>2`. It now allows an explicit root-dimension value group only when counting root rows, with no identity grouping or `include_empty`; related counts and zero-preserving groups still require identity. With or without HAVING, the Model's declared canonical wire set guards value-group source rows: otherwise unknown codes collapse into a projected NULL group and may form a false business bucket. SQLite verifies physical severity `"1"` events yield `critical=3`, `"2"` events yield `major=2` without HAVING, integer cleared-status codes `0`/`1` group correctly, and change-type code `1` yields `add=3` above the threshold. Unknown text and integer codes do not create groups. An ungrouped threshold, child projection and unrelated joins remain rejected. |
| Ordering alarms by a business severity | Q0246-Q0248 | The IC value domain maps physical SEVERITY `"1".."4"` to `critical/major/minor/warning`; Graph formerly sorted the projected text IDs lexically. An explicit field-level `canonical_order(["critical","major","minor","warning"])` now declares severity rank independently of physical codes and mapping declaration order. Preparation rejects missing, duplicate or foreign IDs. Raw Graph `order_by` lowers to a bound closed CASE rank, while projection remains the canonical business ID; known-wire filtering excludes unknown values whose rank is undefined. The same ordered IDs appear in dimension knowledge. SQLite verifies ascending and descending ranks with an unknown wire and a separate alarm-name filter; `take=2` uses complete alarm identity to stabilize same-rank rows, while unrestricted sorting makes no same-rank ordering promise. Undeclared canonical, enum and boolean orderings still diagnose instead of inventing a business order. |
| Alarm-name suffix count | Q0252; Q0253-Q0257 remain field-specific probes | `CurrentAlarm.ALARMNAME` now authorizes `EndsWith` in addition to exact equality; the IC profile declares the closed `Substr`, `Length` and `Sub` scalars required to build this predicate. The suffix text stays bound rather than entering the SQL template. SQLite verifies case-insensitive end matching, rejects a prefix or a longer suffix, and ignores NULL; an undeclared suffix on `alarm_tenant_id` fails before SQL. A standalone ontology preparation test rejects an authorized suffix when the profile lacks `Substr`. No new ontology operator was needed: this was a concrete IC Model authorization/profile gap, not a blanket grant to all text dimensions. |
| Tenant-scoped devices with uncleared alarms | Q0193-Q0195 | Count Device roots after the named Safe Device-to-Tenant relation and qualify each device by a correlated EXISTS over the named `(MEDN,TENANTID)` Alarm-to-Device relation. The SQLite fixture has two uncleared alarms for one WAC, a same-ID foreign-tenant alarm for an otherwise cleared AC, an additional eligible AC and a non-WAC: exactly two devices qualify, regardless of event multiplicity. A direct Alarm JOIN while counting Device roots is rejected as grain-unsafe. Separate `ic/info` requests expose both relations from their owning datasets. The corpus target SQL uses `COUNT(DISTINCT d.id)` and joins alarms by MEDN alone; neither shortcut establishes the pressure model's declared tenant-scoped Device identity. Existing ontology relationships and EXISTS suffice; no new operator is required. |
| Link port-name containment and complement | Q0275 and an adjacent negative intent | IC publishes `EnterprisePhysicalLink.zPortName`; the Model authorizes `Contains` and `NotContains` for this dimension without granting negative containment to unrelated link IP fields. Ontology previously applied `Lower` only for `Contains`, so a differently cased match wrongly passed `NotContains`. Both now use the same closed ASCII case normalization and bound needle; SQL NULL remains unknown and is admitted by neither. A SQLite fixture distinguishes mixed-case matches, unmatched/empty text and NULL. Both SQL dialects lower the same AST (`lower` in SQLite, ASCII `translate` in PostgreSQL); wrong-dimension use is diagnosed and the two operators are discoverable in the dimension knowledge. The negative intent is a semantic complement probe, not a claim that Q0275 itself requests negation. |
| Site-qualified interface operational states | Q0360 | IC's published `I_EnterpriseNetworkLTP.operState` maps `active/inactive/unknown` to `up/down/unknown`; the pressure model now declares this as a Port-owned canonical dimension. Root at Port and qualify through a correlated Port→Device Safe, Device→Site FanOut OR EXISTS with AP classification and online site type. SQLite preserves two same-name/same-status interfaces as separate rows and does not duplicate either when the Device matches both site alternatives; cross-tenant same-ID devices and unmapped status wires are excluded. Directly joining a Site while projecting Port rows is diagnosed as root-grain multiplication. The Port knowledge point references its status and Device ownership. The corpus SQL groups by Device name and Port state, potentially merging distinct interfaces; that grouping is not an interface-identity contract. Existing ontology multi-hop EXISTS and canonical mapping suffice; no new operator is required. |
| Repairable canonical ID diagnostic | Q0193 WAC classification intent | The IC Model publishes `WAC`, not lowercase `wac` or a physical wire such as `AC`. The closed canonical lookup already rejected unknown IDs, but its message omitted the valid IDs. The structured `fail!` now names the dimension, rejected input, every Model-declared stable ID and its exact `ic/info` topic; the service diagnostic retains model-source provenance. A Graph regression checks the candidate list. This does not loosen the accepted intent language, infer aliases or expose physical codes as stable IDs. |

The declared severity order now also supports a separate per-device alarm Top-N
capability probe; Q0246-Q0248 themselves require global ordering, not this
per-owner shape. `top_per` ranks the raw alarm instances by a closed CASE over
the declared canonical IDs, partitions by complete Device identity, and drops
unknown wires even when severity is not projected. A SQLite row fixture verifies
independent tenant partitions, severity priority over physical wire order and
exclusion of unknown codes. Unordered canonical dimensions still fail. The same
fixture exposed and fixed a QueryAst SQLite renderer bug: outer partition order
must reference a named projection's actual alias, not a nonexistent `__q_0`.
| Raw canonical projection without sorting | Q0234-Q0235 severity value domain; Q0246-Q0248 raw severity display | Raw Graph projection already translated declared alarm severity wires into business IDs, but unrecognized wires and SQL NULL were emitted as `NULL` rows whenever the intent neither grouped nor ordered by severity. Every projected canonical dimension now constrains its source to declared wires, independently of count/HAVING/order. SQLite verifies unsorted raw output contains `critical` and `major`, excluding unknown and NULL physical codes; grouped threshold and ordered raw fixtures still pass. This prevents an undeclared physical value masquerading as a business bucket. It does not diagnose the source data quality issue or imply a new valid business value. |
...
| Tenant counts component-qualified server identities | Q0428 | The query counts complete rack Server identities per Tenant; matching Huawei Fans qualify each Server through correlated EXISTS. `fan_parent_server` now includes both `parentResId=oriResId` and fan/server tenant equality, so forgetting an extra tenant constraint cannot borrow a same-parent fan from another tenant. SQLite checks two same-name red servers count as two, two red fans on one server count it once, and the same-ID blue server with a non-Huawei fan does not borrow a red fan. IC's published fan link contains only the parent resource equality: the added tenant equality is an explicit business-ownership pressure contract to validate against production IDs/data, not a claim about the published physical link. The corpus SQL counts DISTINCT display names and groups by the name, which is not an identity count and cannot serve as the business definition. |
