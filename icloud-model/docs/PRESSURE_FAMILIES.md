...
| Endpoint device qualified by site | Q0350-Q0353, Q0366-Q0368, Q0374-Q0379 | Link A/Z roles must each match the Device's complete `(id,tenant_id)` identity; Device→Site permits two independently matching OR keys, then Site→Tenant and Link→Tenant must name the same tenant. `qualify_link_site` encloses both A/Z alternatives and Site alternatives in one three-hop EXISTS so a link is returned/counted once even when both ends and multiple sites qualify. IC's original entity link only publishes id equality, so the added tenant equality is an explicit pressure-model contract needing production mapping confirmation. The representative Q0374 fixture excludes another tenant's same device ID and split-site tenant filters. Device-name projection alongside link rows remains a separate role-context question. |
| Endpoint device qualified by site | Q0350-Q0353, Q0366-Q0368, Q0374-Q0379 | Link A/Z roles must each match the Device's complete `(id,tenant_id)` identity; Device→Site permits two independently matching OR keys, then Site→Tenant and Link→Tenant must name the same tenant. `qualify_link_site` encloses both A/Z alternatives and Site alternatives in one three-hop EXISTS so a link is returned/counted once even when both ends and multiple sites qualify. The directed peer hub explicitly binds those A/Z Safe relations and carries the full participant grain into its EXISTS; absent tenant equality is diagnosed. IC's original entity link only publishes id equality, so the added tenant equality is an explicit pressure-model contract needing production mapping confirmation. Device-name projection alongside link rows remains a separate role-context question. |
...
| Component-qualified event observation | Q0392-Q0397 | Event→Server uses the declared Safe `(MEDN,TENANTID)` ownership relation; a possibly non-unique Fan→Server `parentResId=oriResId` relation stays FanOut, admitted only inside correlated EXISTS with independently aligned Fan/Server tenant keys. Event.CSN is not duplicated by two qualifying fans or two servers with the same oriResId. For per-server event counts, the Model declares Server grain `(id,tenant_id)` and lowering hides both identity keys in GROUP BY while exposing the requested name. Q0392/Q0396 test fan qualification and grouping with externally supplied UTC bounds; power-supply and other component carriers remain separate probes. The alarm time column is `OCCURUTC`, not `OCCUR_UTC`. |
...
| Upper-bounded matching sample count | Q0062-Q0064 | `qualify_sample_count` explicitly declares nonnegative inclusive `min_matches`/`max_matches`, raw sample predicate and UTC window. `NOT EXISTS` with `COUNT > max` includes zero when min is zero; an additional grouped EXISTS enforces a positive minimum. The SQLite fixture covers 0/1/4/5 matches and same resource ID across tenants. Natural language "fewer than five" includes zero; corpus INNER JOIN excludes it. The agent/domain must select `0..4` or `1..4` explicitly rather than infer that discrepancy from SQL. |
