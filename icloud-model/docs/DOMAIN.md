...
| Raw KPI samples can be ranked separately for each owner | `sample_top` uses the named Safe sample-to-device relation and device `(id,tenant_id)` grain as hidden partition keys; metric `(res_id,tenant_id,ts)` grain breaks ranking ties | Q0036-shaped Top 3 does not partition by a repeated display name or expose keys; interface Top 3 with tenant/site filters requires a separate composed-model acceptance. |
