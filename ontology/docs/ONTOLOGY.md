...
`sample_epoch_top` 在已声明的指标样本 grain 和完整 owner 身份上执行逐主体 Top-N；
`latest_epoch_sample` 用指标数据集的权威 `UtcEpochMillis` 时钟排序完整样本行，
可指定同一权威字段上的毫秒窗口，或不指定窗口而查询全历史。
显式选择另一个 UTC 时钟作为“最新”的窗口会被诊断。两者沿用有名安全关系、
隐藏 owner grain 分区及稳定的完整样本 grain 排序键。
`qualify_epoch_samples` 与 `qualify_epoch_sample_count` 分别支持按原始样本谓词
和最小匹配次数筛选主体、按包含零次的样本计数区间筛选主体；每个条件在单独的相关
聚合 EXISTS 中使用自己的整数时间窗口及完整 owner 关联键。
样本资格窗口必须指向指标数据集的权威 UTC 毫秒时钟，不能混入次级时钟或文本时间。
`qualify_epoch_metrics` 对每项声明的指标度量在独立 UTC 毫秒窗口内应用原始样本过滤、
聚合及 HAVING，再按完整主体身份取交；既不能覆盖该度量在 Model 中声明的聚合方式，
也不能把不同指标样本集相互 JOIN 后再聚合。
`observe_epoch_qualified` 先在各自的 UTC 毫秒资格窗口中计数样本，
再对同一完整主体身份在独立的观察窗口中列出原始样本或聚合已声明的度量。
观察根保持指标样本数据集，聚合投影按隐藏 owner grain 分组；组件过滤与组件上下文
继续复用相同的关系约束。观察窗口和资格窗口都必须指向各自数据集的权威 UTC 毫秒时钟。
`observe_epoch_event_component` 在权威 UTC 毫秒事件窗口内观察事件行或按完整 owner
身份计数，并复用组件 FanOut EXISTS、事件到 owner 的完整 Safe 键及双方对齐的租户
归属关系；组件匹配不会将同一事件行复制，也不能跨租户借用同 ID 的主体。
`observe_related`/`observe_related_epoch` 使用同一有名 Safe 样本到 owner 的观察路径，
但不虚构资格条件。窗口可选：未指定时聚合或列出全历史样本；指定时必须在所选编码
的样本数据集时间字段上降为半开区间。带 owner 显示维度的聚合保留完整隐藏身份，
度量聚合方式只能选 Model 声明的指标或样本汇总知识点。
...
UNION 分支的固定 Eq 过滤值也在准备阶段匹配逻辑字段类型：文本、整数、浮点
分别使用相应输入；枚举字段仅接受声明的稳定值，不依赖数据库隐式转换来解释
分支的业务行集。
